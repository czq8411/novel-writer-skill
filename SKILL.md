...
description: |
  ...
  v20.1新增：基础设定完整性检测（灵感构思/大纲创作/诊断复盘三阶段联动），主动识别并提示世界观背景、核心冲突类型、角色设定、情节转折点、金手指规则、势力分布、货币体系、修炼体系等8大基础设定缺失。
...
> **快速导航**：`灵感/构思`→模式A | `大纲`→模式B | `细纲`→模式B-2 | `写正文/继续`→模式C | `书名/简介`→模式D | `诊断/复盘`→模式E | `一键创作`→模式F
...
> **加载机制**：根据触发词自动加载对应模式子文件（`modes/`目录），避免全量加载。
